...
/**
 * The {@code PetSelection} class provides a graphical interface for selecting a pet in the Virtual Pet Application.
 * It allows users to view available pets, see their stats, and select a pet to adopt.
 * 
 * <p>The class features a visually appealing design with custom colors, hover effects, and dialog boxes
 * for displaying pet information and confirming selections.
 * 
 * @author Hamza Khan, Omar Hossain
 *  
 */
public class PetSelection {
    ...
    // Custom colors to match the pastoral theme
    ...
    /**
     * Displays the pet selection screen, allowing the user to choose a pet.
     * 
     * @param onPetSelected a {@code Consumer<Pet>} callback that is invoked when a pet is selected
     */
    public static void selectPet(Consumer<Pet> onPetSelected) {
        ...
        // Create the main frame for the pet selection screen
        ...
        // Create a panel to center the pet options
        ...
        // Loop through the pets and create UI components for each
        for (int index = 0; index < pets.length; index++) {
            ...
            if (isLastPet) {
                ...
                selectButton.addActionListener(e -> showLockedPetDialog(frame));
            } else {
                // Handle unlocked pets
    ...
    /**
     * Creates a pastoral-themed button with custom styling and hover effects.
     * 
    ...
    private static JButton createPastoralButton(String text) {
        ...
        // Rounded border with a rustic style
    ...
    // Method to show locked pet dialog
    private static void showLockedPetDialog(JFrame frame) {
