 ...
 * 
 * @author Omar Hossain, Hamza Khan
 */
public class ParentalControls extends JDialog {
    ...
    private CardLayout cardLayout; // Layout manager for switching between password and settings screens
    private JPanel mainPanel; // Main container panel
    private static final String PASSWORD = "123"; // Default password for accessing parental controls

    // Parental control settings
    private boolean timeLimitEnabled = false; // Whether time restrictions are enabled
    private LocalTime startTime = LocalTime.of(8, 0); // Default start time (8:00 AM)
    private LocalTime endTime = LocalTime.of(20, 0);  // Default end time (8:00 PM)
    private long totalPlayTime = 0; // Total playtime in seconds
    private long averagePlayTime = 0; // Average playtime per session in seconds
    private int sessionsCount = 0; // Number of play sessions

    // UI components for settings
    private JCheckBox toggleLimit; // Checkbox to enable/disable time restrictions
    private JTextField startTimeField; // Input field for start time
    private JTextField endTimeField; // Input field for end time
    private JLabel totalPlayTimeLabel; // Label to display total playtime
    private JLabel avgPlayTimeLabel; // Label to display average playtime
    private JLabel sessionsLabel; // Label to display the number of sessions

    private static final String CONTROLS_FILE = "saves/parental_controls.dat"; // File path for saving settings
    public static final DateTimeFormatter TIME_FORMATTER = DateTimeFormatter.ofPattern("HH:mm"); // Time format

    /**
     * Default constructor for ParentalControls.
     * Initializes the dialog without a parent frame.
     * @author Hamza Khan
     */
    ...
    /**
     * Constructor for ParentalControls with a parent frame.
     * 
    ...
    public ParentalControls(JFrame parent) {
        ...
        setSize(700, 500); // Set the size of the dialog
        setLocationRelativeTo(parent); // Center the dialog relative to the parent
        setDefaultCloseOperation(DISPOSE_ON_CLOSE); // Dispose the dialog on close
        setUndecorated(true); // Remove window decorations for a custom look
        ...
        // Add password and settings panels to the main panel
        mainPanel.add(createPasswordPanel(), "password");
        mainPanel.add(createSettingsPanel(), "settings");

        // Set the main panel as the content pane
        setContentPane(mainPanel);
        cardLayout.show(mainPanel, "password"); // Show the password panel initially

        // Add a window listener to save settings when the dialog is closed
    ...
    /**
     * Creates the password panel for accessing parental controls.
     * 
     * @return A JPanel containing the password input UI.
     * @author Hamza Khan
     */
    private JPanel createPasswordPanel() {
        ...
        titleLabel.setBounds(0, 50, 700, 50); // Position and size
        ...
        exitButton.addActionListener(e -> dispose()); // Close the dialog on click
        ...
        confirmButton.addActionListener(e -> {
            ...
            if (enteredPassword.equals(PASSWORD)) {
                cardLayout.show(mainPanel, "settings"); // Show the settings panel if the password is correct
            } else {
    ...
    /**
     * Creates the settings panel for managing parental controls.
     * 
     ...
     * @return A JButton styled with custom properties.
     * @author Omar Hossain
     */
    ...
    /**
     * Validates the time format and saves the time settings.
     * 
     ...
     * @return A formatted string in HH:mm:ss format.
     * @author Omar Hossain
     */
    ...
    /**
     * Updates the statistics labels with the current values.
     * 
     * 
     */
    ...
    /**
     * Saves parental control settings to a file.
     * 
     * @author Omar Hossain
     */
    ...
    /**
     * Loads parental control settings from a file.
     * 
     * @author Omar Hossain
     */
    ...
    /**
     * Checks if playing is allowed at the current time.
     * 
    ...
    /**
     * Loads parental control settings statically without creating an instance.
     * 
     * @return A ParentalSettings object or null if not found.
     * @author Omar Hossain
     */
    ...
    /**
     * Updates playtime statistics with a new session duration.
     * 
    ...
    /**
     * Saves parental control settings statically without creating an instance.
     * 
     ...
     * This can be called from the user interface to verify settings.
     * 
     * @author Omar Hossain
     */
    ...
    /**
     * Revives a pet by loading its save file and resetting its state.
     * 
     * @author Omar Hossain
     */
